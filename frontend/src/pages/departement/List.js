...
const DepartementListPage = (props) => {
		const app = useApp();
	const filterSchema = {
		search: {
			tagTitle: "Search",
	...
	function ActionButton(data){
		...
		{
			label: "View",
			command: (event) => { app.navigate(`/departement/view/${data.id}`) },
			icon: "pi pi-eye"
		},
		{
			label: "Edit",
			command: (event) => { app.navigate(`/departement/edit/${data.id}`) },
			icon: "pi pi-pencil"
		},
		{
			label: "Delete",
	...
	function IdTemplate(data){
		if(data){
			return (
				<Link to={`/departement/view/${data.id}`}> { data.id }</Link>
			);
		}
	}
	...
	return (
<main id="DepartementListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title="Departement"   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <Link to={`/departement/add`}>
...
DepartementListPage.defaultProps = {
	primaryKey: 'id',
	pageName: 'departement',
	apiPath: 'departement/index',
	routeName: 'departementlist',
