...
const CarburantListPage = (props) => {
	...
	// return sum of montant for the current records
	const totalMontant = utils.sum(records, 'montant');
	function ActionButton(data){
		const items = [
		{
			label: "View",
			command: (event) => { app.navigate(`/carburant/view/${data.id}`) },
			icon: "pi pi-eye"
		},
		{
			label: "Edit",
			command: (event) => { app.navigate(`/carburant/edit/${data.id}`) },
			icon: "pi pi-pencil"
		},
		{
			label: "Delete",
	...
	function IdTemplate(data){
		if(data){
			return (
				<Link to={`/carburant/view/${data.id}`}> { data.id }</Link>
			);
		}
	}
	function VehicleHeaderTemplate(data){
		if(data){
			return (
					<strong>
	...
	function VehicleTemplate(data){
		if(data){
			return (
				<>{data.vehicle && <Button className="p-button-text" icon="pi pi-eye" label={data.vehicule_immatricule} onClick={() => app.openPageDialog(<VehiculeViewPage isSubPage apiPath={`/vehicule/view/${data.vehicle}`} />, {closeBtn: true })} /> }</>
			);
		}
	}
	...
	return (
<main id="CarburantListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title="Carburant"   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <Link to={`/carburant/add`}>
                        ...
                        <div >
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column  field="vehicle" header={VehicleHeaderTemplate} body={VehicleTemplate}  ></Column>
                                ...
                                <div className="flex justify-content-around font-bold p-3">
                                    <div>Total Montant : <Chip label={totalMontant} /></div>
                                </div>
                            </div>
                            <PageFooter />
                        </div>
...
CarburantListPage.defaultProps = {
	primaryKey: 'id',
	pageName: 'carburant',
	apiPath: 'carburant/index',
	routeName: 'carburantlist',
...
export default CarburantListPage;
