...
const SocieteListPage = (props) => {
	...
	function ActionButton(data){
		...
		{
			label: "View",
			command: (event) => { app.navigate(`/societe/view/${data.id}`) },
			icon: "pi pi-eye"
		},
		{
			label: "Edit",
			command: (event) => { app.navigate(`/societe/edit/${data.id}`) },
			icon: "pi pi-pencil"
		},
		{
			label: "Delete",
	...
	function IdTemplate(data){
		if(data){
			return (
				<Link to={`/societe/view/${data.id}`}> { data.id }</Link>
			);
		}
	}
	...
	return (
<main id="SocieteListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title="Societe"   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
...
SocieteListPage.defaultProps = {
	primaryKey: 'id',
	pageName: 'societe',
	apiPath: 'societe/index',
	routeName: 'societelist',
